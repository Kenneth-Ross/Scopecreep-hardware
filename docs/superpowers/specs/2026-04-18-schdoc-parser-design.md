...
Synthesized prose paragraph assembled from parsed data:
- Board name / MCU (from components matching `A*` or `U*` with "Nucleo"/"STM32" in description)
- Power supply chain (source rail → converter → output rails)
- Functional blocks (zone names + brief component summary)
- External interfaces (connector count, signal names from ports)
- Test points available

No templates — built by string assembly from structured fields.
...
- Standard library only beyond that (`dataclasses`, `pathlib`, `struct`, `re`)

No new dependencies added to `python/requirements.txt` beyond `olefile`.
